...
namespace Nito.Collections
{
    ...
    public sealed class Deque<T> : IList<T>, IReadOnlyList<T>, System.Collections.IList
    {
        ...
        {
            public static IReadOnlyCollection<T1> ReifyCollection<T1>(IEnumerable<T1> source)
            {
                if (source == null)
                    throw new ArgumentNullException(nameof(source));

                if (source is IReadOnlyCollection<T1> result)
                    return result;
                if (source is ICollection<T1> collection)
                    return new CollectionWrapper<T1>(collection);
                if (source is ICollection nongenericCollection)
                    return new NonGenericCollectionWrapper<T1>(nongenericCollection);

                return new List<T1>(source);
            }

            private sealed class NonGenericCollectionWrapper<T1> : IReadOnlyCollection<T1>
            {
            ...
            private sealed class CollectionWrapper<T1> : IReadOnlyCollection<T1>
            {
                private readonly ICollection<T1> _collection;

                public CollectionWrapper(ICollection<T1> collection)
                {
                    _collection = collection ?? throw new ArgumentNullException(nameof(collection));
                }
