...
//
// Copied from https://source.dot.net/#System.Collections/System/Collections/Generic/PriorityQueue.cs,6668c6ef2948ff6e
//
// Modifications:
//   added Remove(T) method to be able to remove of elements from the middle of the queue (not just the root one)
...
namespace System.Collections.Generic
{
    ...
    public class PriorityQueue<TElement, TPriority>
    {
        ...
        public PriorityQueue(int initialCapacity, IComparer<TPriority>? comparer)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(initialCapacity), initialCapacity, "Value must be non-negative.");
            }
        ...
        public int EnsureCapacity(int capacity)
        {
            if (capacity < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(capacity), capacity, "Value must be non-negative.");
            }
        ...
        public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, ICollection
        {
            ...
            void ICollection.CopyTo(Array array, int index)
            {
                ...
                if (array.Rank != 1)
                {
                    throw new ArgumentException("Multidimentional arrays not supported", nameof(array));
                }
                ...
                {
                    throw new ArgumentOutOfRangeException(nameof(index), index, "index must be non-negative and less than array length.");
                }

                if (array.Length - index < _queue._size)
                {
                    throw new ArgumentException("There is not enough space in array");
                }
        ...
        internal sealed class PriorityQueueDebugView<TElement1, TPriority1>
        {
            private readonly PriorityQueue<TElement1, TPriority1> _queue;
            private readonly bool _sort;

            public PriorityQueueDebugView(PriorityQueue<TElement1, TPriority1> queue)
            {
                _queue = queue ?? throw new ArgumentNullException(nameof(queue));
                _sort = true;
            }

            public PriorityQueueDebugView(PriorityQueue<TElement1, TPriority1>.UnorderedItemsCollection collection)
            {
                _queue = collection?._queue ?? throw new ArgumentNullException(nameof(collection));
            }

            [DebuggerBrowsable(DebuggerBrowsableState.RootHidden)]
            public (TElement1 Element, TPriority1 Priority)[] Items
            {
                get
                {
                    var list = new List<(TElement1 Element, TPriority1 Priority)>(_queue.UnorderedItems);
                    if (_sort)
                    {
